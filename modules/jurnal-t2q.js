/**
 * Modul Jurnal T2Q Tahunan - SDIT Al-Kautsar
 * Menampilkan rincian capaian SMT 1, SMT 2, & Rata-rata T2Q Tahunan per guru dengan Freeze Header & Dynamic Color
 */

async function initJurnalT2qModule() {
  $('#pageTitle').text('Rincian Jurnal T2Q Tahunan');
  ...
  contentArea.html(`
    ...
    <div class="card card-outline card-teal">
      <div class="card-header">
        <h3 class="card-title"><i class="fas fa-book-open mr-2"></i>Data Jurnal T2Q Tahunan Guru</h3>
        <div class="card-tools">
          <button type="button" class="btn btn-tool" onclick="initJurnalT2qModule()">
      ...
      <div class="card-body p-0">
        <div id="tableSpinner" class="text-center my-4 p-3">
          ...
          <p class="mt-2 text-muted">Memuat data Jurnal T2Q Tahunan...</p>
        </div>
        
        <!-- Wadah Tabel dengan Freeze Header -->
        <div class="table-responsive table-freeze-container">
          <table id="tableJurnalT2q" class="table table-bordered table-striped table-hover mb-0" style="width:100%">
  ...
  try {
    const url = `${API_URL}?action=read&sheet=${encodeURIComponent('Jurnal T2Q')}`;
    ...
    if (result.status === 'success' && Array.isArray(result.data)) {
      renderTableJurnalT2q(result.data);
    } else {
      $('#tbodyJurnalT2q').html(`<tr><td colspan="5" class="text-center text-muted p-3">Gagal memuat data atau sheet kosong.</td></tr>`);
    }
  } catch (error) {
    console.error('Error Jurnal T2Q:', error);
    $('#tableSpinner').addClass('d-none');
    $('#tbodyJurnalT2q').html(`<tr><td colspan="5" class="text-center text-danger p-3">Terjadi kesalahan saat terhubung ke server.</td></tr>`);
  }
}

function renderTableJurnalT2q(rawData) {
  if (rawData.length <= 2) {
    $('#tbodyJurnalT2q').html(`<tr><td colspan="5" class="text-center text-muted p-3">Tidak ada data ditemukan.</td></tr>`);
    return;
  }
  ...
  for (let i = 2; i < rawData.length; i++) {
    ...
    const badgeSmt1 = generatePercentBadgeT2q(smt1);
    const badgeSmt2 = generatePercentBadgeT2q(smt2);
    const badgeRataRata = generatePercentBadgeT2q(rataRata);
  ...
  $('#tbodyJurnalT2q').html(rowsHtml);

  if ($.fn.DataTable) {
    if ($.fn.DataTable.isDataTable('#tableJurnalT2q')) {
      $('#tableJurnalT2q').DataTable().destroy();
    }
    $('#tableJurnalT2q').DataTable({
      responsive: true,
      paging: false,
      info: false,
      searching: true,
